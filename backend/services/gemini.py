...
class GeminiService:
    ...
    async def _generate_json(self, prompt: str, model: str | None = None, max_tokens: int = 4096) -> dict | list:
        ...
        text = re.sub(r"```", "", text)
        # Skip any preamble text and find the actual JSON start
        ...
        try:
            ...
            pass
        # JSON may be truncated: recover complete objects by finding last "},\n"
        for sep in ["},\n", "},\r", "},"]:
            ...
            if last_complete > 0:
                ...
                try:
                    ...
                    pass
        print(f"[JSON UNRECOVERABLE] {repr(text[:300])}", flush=True)
    ...
    async def generate_simulacre(self, temes: list[dict], seed: str) -> list[dict]:
        ...
        result = await self._generate_json(prompt, model="llama-3.1-8b-instant", max_tokens=4300)
